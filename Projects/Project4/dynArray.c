...
** Due Date: 03/06/2018
** Description: Main source file for the implementation of a dynamica array
** that also implements a heap-based priority queue.
** Output: 
*********************************************************************/
...
/*	Add a node to the heap
	...
	Citation: code for this function is heavily based on the code made available
	on the OSU CS 261 video lecture material.
*/
void addHeap(DynArr *heap, TYPE node)
{
	...
	//initialize variables and add node to the very end of the array
	int parent;
	int pos = sizeDynArr(heap) - 1;
	
	

	//adjust that new value to fit it in the correct location
	while (pos > 0) {
		parent = (pos - 1) / 2;
		//check if the value at pos is less than the parent value -1 denotes that
		//the value on the LHS is less than the RHS
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rightChild < max) {
		smallest = _smallerIndexHeap(heap, leftChild, rightChild);
	
		
		if (compare(getDynArr(heap, smallest), getDynArr(heap, pos)) == -1) {
			swapDynArr(heap, smallest, pos);

			_adjustHeap(heap, max, smallest);
		}
		
	}
	//case in which there is only one child/check the left child next if right child passed check
...
/* builds a heap from an arbitrary dynArray
	...
	citation: OSU CS 261 Worksheet 34
 */
